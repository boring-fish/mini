<template>
  <div id="navListbar">
    <div
      id="top_tabs_box"
      class="layui-tab marg0"
      lay-filter="bodyTab"
    >
      <ul
        id="top_tabs"
        class="layui-tab-title top_tab"
      >
        <!-- class="layui-this" 显示高亮 -->
        <!-- 根据当前的页面添加class：依据是路由的名字的组件的名字 -->
        <li :class="{'layui-this':'ContentArticle' == $route.name}">
          <router-link :to="{name:'ContentArticle'}">
            <i class="iconfont icon-text" /><cite>列表</cite>
          </router-link>
        </li>
        <li
          v-for="(data,index) in currentPage"
          :key="index"
          :class="{'layui-this':data.name == $route.name}"
        >
          <router-link :to="{name:data.name}">
            <i class="iconfont icon-text" /><cite>{{ data.text }}</cite>
          </router-link>
          <i
            class="layui-icon layui-unselect layui-tab-close"
            @click="closePage(data)"
          >ဆ</i>
        </li>
      </ul>
    </div>
